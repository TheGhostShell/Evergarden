...
public class RouteBuilder {
    ...
    private ArrayList<RouteAgregate>       routeAggregator;
    private String                         hash;
    private RouterFunction<ServerResponse> routes;

    @Autowired
    public RouteBuilder() {
        this.logger = LoggerFactory.getLogger(RouteBuilder.class);
        routeAggregator = new ArrayList<>();
    }
    ...
    public RouterFunction<ServerResponse> build() throws NoSuchAlgorithmException, NoSuchMethodException,
        ...
        routes = RouterFunctions.route(RequestPredicates.GET("/" + hashed), serverRequest -> {
        ...
        for (RouteAgregate route : routeAggregator) {
            ...
            routes = RouterFunctions.route((RequestPredicate) method.invoke(obj, path), handler).and(routes);
        }
